...
document.querySelectorAll('.action-btn.view-btn').forEach(btn => {
    btn.addEventListener('click', async function() {
        ...
        try {
            const response = await fetch(`http://localhost/MedCeylon/hospital/get-patient-details?id=${patientId}`);
...
// View Medical History
        document.querySelectorAll('.action-btn.history-btn').forEach(btn => {
            btn.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch(`http://localhost/MedCeylon/hospital/get-medical-history?id=${patientId}`);
                    if (!response.ok) {
                        ...
                        data.forEach(record => {
                            historyHtml += `
                                <div class="history-item">
                                    <div class="history-date"><p><strong>Appointment Date:</strong>${new Date(record.appointment_date).toLocaleDateString()}</div>
